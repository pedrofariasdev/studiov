...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const createBrandButton = document.getElementById(
        "create-brand-button"
    );
    ...
    const brandModalTitle = document.getElementById(
        "brand-modal-title"
    );

    const brandModalClose = document.getElementById(
        "brand-modal-close"
    );

    const brandFormCancel = document.getElementById(
        "brand-form-cancel"
    );
    ...
    const brandNameError = document.getElementById(
        "brand-name-error"
    );

    const brandIndustryInput = document.getElementById(
        "brand-industry"
    );

    const brandLogoInput = document.getElementById(
        "brand-logo-url"
    );

    const brandLanguageInput = document.getElementById(
        "brand-language"
    );

    const brandWebsiteInput = document.getElementById(
        "brand-website"
    );
    ...
    const brandFormError = document.getElementById(
        "brand-form-error"
    );
    ...
    const brandFormSubmitButton =
        brandForm?.querySelector(
            'button[type="submit"]'
        );

    const brandFormSubmitLoader =
        brandFormSubmitButton?.querySelector(
            ".button-loader"
        );
    ...
    const brandsLoading = document.getElementById(
        "brands-loading"
    );

    const brandsEmpty = document.getElementById(
        "brands-empty"
    );

    const brandsNoResults = document.getElementById(
        "brands-no-results"
    );

    const brandsGrid = document.getElementById(
        "brands-grid"
    );

    const totalBrandsCount = document.getElementById(
        "total-brands-count"
    );

    const activeBrandsCount = document.getElementById(
        "active-brands-count"
    );

    const archivedBrandsCount = document.getElementById(
        "archived-brands-count"
    );
    ...
    async function initializeWorkspaceContext() {
        ...
        try {
            ...
            console.log(
                "Contexto do dashboard carregado:",
                {
                    userId: user.id,
    ...
    function updateBrandCounters() {
        ...
        const archivedCount = brands.filter(
        ...
        filteredBrands.sort(
            (firstBrand, secondBrand) => {
                if (selectedSort === "oldest") {
                    return (
                        new Date(
                            firstBrand.created_at
                        ).getTime() -
                        new Date(
                            secondBrand.created_at
                        ).getTime()
                    );
                }
            ...
            const logoContent = logoUrl
                ? `
                    <img
                        ...
                        alt="Logótipo da marca ${escapeHtml(
                            brand.name || ""
                        )}"
                        loading="lazy"
    ...
    function setGeneralFormError(message) {
        ...
        brandFormError.classList.remove(
            "hidden"
        );
    }
    ...
    function clearFormErrors() {
        ...
        brandLogoInput?.classList.remove(
            "is-invalid"
        );

        primaryColorInput?.classList.remove(
            "is-invalid"
        );

        secondaryColorInput?.classList.remove(
            "is-invalid"
        );
    ...
    function setBrandFormSubmitting(
        isSubmitting
    ) {
        const isEditing = Boolean(
            brandIdInput?.value.trim()
        );

        if (brandFormSubmitButton) {
            brandFormSubmitButton.disabled =
                isSubmitting;

            brandFormSubmitButton.setAttribute(
                "aria-busy",
                String(isSubmitting)
            );
        }
        ...
        if (brandFormSubmitLoader) {
            brandFormSubmitLoader.classList.toggle(
                "hidden",
                !isSubmitting
            );
        }
    ...
    function setLanguageFormValue(language) {
        ...
        const normalizedLanguage =
            getLanguageForForm(language);

        const optionExists = Array.from(
            brandLanguageInput.options
        ).some(
            option =>
                option.value ===
                normalizedLanguage
        ...
        if (!optionExists) {
            ...
            brandLanguageInput.appendChild(
                newOption
            );
        }
    ...
    function validateBrandForm() {
        ...
        const primaryColor =
            primaryColorInput?.value.trim() ||
            "";

        const secondaryColor =
            secondaryColorInput?.value.trim() ||
            "";
        ...
        if (
            ...
            primaryColorInput?.classList.add(
                "is-invalid"
            );

            setGeneralFormError(
                "A cor principal deve seguir o formato #RRGGBB."
            );
        ...
        if (
            ...
            secondaryColorInput?.classList.add(
                "is-invalid"
            );

            setGeneralFormError(
                "A cor secundária deve seguir o formato #RRGGBB."
            );
    ...
    brandNameInput?.addEventListener(
        "input",
        () => {
            clearFieldError(
                brandNameInput,
    ...
    brandWebsiteInput?.addEventListener(
        "input",
        () => {
            clearFieldError(
                brandWebsiteInput,
    ...
    function openArchiveBrandModal(
        brandId
    ) {
        if (!archiveBrandModal) {
            console.error(
                'Elemento com id="archive-brand-modal" não encontrado.'
            );
    ...
    function setArchiveSubmitting(
        ...
        archiveBrandConfirm.disabled =
            isSubmitting;

        archiveBrandConfirm.setAttribute(
            "aria-busy",
            String(isSubmitting)
        );

        archiveBrandConfirm.textContent =
            isSubmitting
                ? "A arquivar..."
                : "Arquivar marca";
    }
    ...
    async function archiveBrand(brandId) {
        ...
        try {
            ...
            showToast(
                "success",
                "Marca arquivada",
                "A marca foi arquivada com sucesso."
            );
        } catch (error) {
            console.error(
            ...
            showToast(
                ...
                "Erro ao arquivar marca",
                error?.message ||
                "Não foi possível arquivar a marca."
            );
        } finally {
    ...
    async function restoreBrand(brandId) {
        try {
            ...
            showToast(
                ...
                "Erro ao restaurar marca",
                error?.message ||
                "Não foi possível restaurar a marca."
            );
        }
    }
    ...
    archiveBrandCancel?.addEventListener(
        "click",
        closeArchiveBrandModal
    );

    archiveBrandModal?.addEventListener(
        "click",
        event => {
            if (
                event.target ===
    ...
    brandsGrid?.addEventListener(
        ...
        async event => {
            ...
            const brandId =
                brandCard?.dataset.brandId;

            const action =
                actionButton.dataset.brandAction;
    ...
    document.addEventListener(
        ...
        event => {
            ...
            if (
                archiveBrandModal &&
                archiveBrandModal.classList.contains(
                    "is-open"
                )
            ) {
                closeArchiveBrandModal();
