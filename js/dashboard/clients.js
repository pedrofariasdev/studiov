...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const clientsLoading = document.getElementById(
        "clients-loading"
    );

    const clientsEmpty = document.getElementById(
        "clients-empty"
    );

    const clientsNoResults = document.getElementById(
        "clients-no-results"
    );
    ...
    const totalClientsCount = document.getElementById(
        "total-clients-count"
    );

    const activeClientsCount = document.getElementById(
        "active-clients-count"
    );

    const archivedClientsCount = document.getElementById(
        "archived-clients-count"
    );

    const clientsSearchInput = document.getElementById(
        "clients-search-input"
    );

    const clientsStatusFilter = document.getElementById(
        "clients-status-filter"
    );

    const clientsSortSelect = document.getElementById(
        "clients-sort-select"
    );

    const clearClientFiltersButton = document.getElementById(
        "clear-client-filters-button"
    );
    ...
    const emptyCreateClientButton = document.getElementById(
        "empty-create-client-button"
    );
    ...
    const clientModalClose = document.getElementById(
        "client-modal-close"
    );
    ...
    const clientNameError = document.getElementById(
        "client-name-error"
    );

    const clientCompanyInput = document.getElementById(
        "client-company-name"
    );
    ...
    const clientEmailError = document.getElementById(
        "client-email-error"
    );
    ...
    const clientWebsiteInput = document.getElementById(
        "client-website"
    );

    const clientWebsiteError = document.getElementById(
        "client-website-error"
    );

    const clientNotesInput = document.getElementById(
        "client-notes"
    );

    const clientNotesCounter = document.getElementById(
        "client-notes-counter"
    );

    const clientFormError = document.getElementById(
        "client-form-error"
    );

    const clientFormSubmitText = document.getElementById(
        "client-form-submit-text"
    );

    const clientFormSubmitButton = document.getElementById(
        "client-form-submit"
    );

    const clientFormLoader = document.getElementById(
        "client-form-loader"
    );

    const archiveClientModal = document.getElementById(
        "archive-client-modal"
    );

    const archiveClientCancel = document.getElementById(
        "archive-client-cancel"
    );

    const archiveClientConfirm = document.getElementById(
        "archive-client-confirm"
    );

    const clientBrandsModal = document.getElementById(
        "client-brands-modal"
    );

    const clientBrandsModalClose = document.getElementById(
        "client-brands-modal-close"
    );

    const clientBrandsClientName = document.getElementById(
        "client-brands-client-name"
    );

    const clientBrandsLoading = document.getElementById(
        "client-brands-loading"
    );

    const clientBrandsEmpty = document.getElementById(
        "client-brands-empty"
    );

    const clientBrandsList = document.getElementById(
        "client-brands-list"
    );

    const clientBrandsCancel = document.getElementById(
        "client-brands-cancel"
    );

    const clientBrandsSave = document.getElementById(
        "client-brands-save"
    );
    ...
    async function initializeWorkspaceContext() {
        ...
        console.log(
            "Contexto de Clientes carregado:",
            {
                userId: user.id,
    ...
    function setClientsView(view) {
        clientsLoading?.classList.toggle(
            "hidden",
            view !== "loading"
        );

        clientsEmpty?.classList.toggle(
            "hidden",
            view !== "empty"
        );

        clientsNoResults?.classList.toggle(
            "hidden",
            view !== "no-results"
        );

        clientsGrid?.classList.toggle(
            "hidden",
            view !== "grid"
        );
    }
    ...
    function updateClientCounters() {
        ...
        if (archivedClientsCount) {
            archivedClientsCount.textContent =
    ...
    function renderClients(items = clients) {
        ...
        items.forEach(client => {
            ...
            const emailHref =
                getSafeMailto(client.email);

            const telephoneHref =
                getSafeTelephone(client.phone);

            const websiteHref =
                getSafeHttpUrl(client.website_url);

            const associatedBrands =
                getClientBrands(client.id);
    ...
    function updateClientNotesCounter() {
        ...
        clientNotesCounter.textContent =
            `${clientNotesInput.value.length}/2000`;
    }
    ...
    function resetClientForm() {
        ...
        if (clientFormSubmitText) {
            clientFormSubmitText.textContent =
                "Criar cliente";
        }
    ...
    function setClientFormSubmitting(isSubmitting) {
        ...
        if (clientFormSubmitButton) {
            clientFormSubmitButton.disabled =
                isSubmitting;

            clientFormSubmitButton.setAttribute(
                "aria-busy",
                String(isSubmitting)
            );
        }

        clientFormLoader?.classList.toggle(
            "hidden",
            !isSubmitting
        );
    ...
    function validateClientForm() {
        ...
        const website =
            clientWebsiteInput?.value.trim() || "";

        if (name.length < 2 || name.length > 120) {
            clientNameInput?.classList.add(
                "is-invalid"
            );
        ...
        if (!isValidEmail(email)) {
            clientEmailInput?.classList.add(
                "is-invalid"
            );
        ...
        if (!isValidWebsite(website)) {
            clientWebsiteInput?.classList.add(
                "is-invalid"
            );
    ...
    emptyCreateClientButton?.addEventListener(
        "click",
        openCreateClientModal
    );

    clientModalClose?.addEventListener(
        "click",
        () => {
    ...
    clientFormCancel?.addEventListener(
        "click",
        () => {
            closeModal(clientModal);
        }
    ...
    clientNotesInput?.addEventListener(
        "input",
        updateClientNotesCounter
    );
    ...
    clientNameInput?.addEventListener(
        "input",
        () => {
            clientNameInput.classList.remove(
                "is-invalid"
            );
    ...
    clientEmailInput?.addEventListener(
        "input",
        () => {
            clientEmailInput.classList.remove(
                "is-invalid"
            );
    ...
    clientWebsiteInput?.addEventListener(
        "input",
        () => {
            clientWebsiteInput.classList.remove(
                "is-invalid"
            );
    ...
    document.addEventListener(
        ...
        event => {
            ...
            if (
                clientBrandsModal &&
                clientBrandsModal.classList.contains(
                    "is-open"
                )
            ) {
            ...
            if (
                archiveClientModal &&
                archiveClientModal.classList.contains(
                    "is-open"
                )
            ) {
    ...
    clientsGrid?.addEventListener(
        ...
        async event => {
            ...
            const clientCard = actionButton.closest(
                "[data-client-id]"
            );
            ...
            const action =
                actionButton.dataset.clientAction;
        ...
        async event => {
            ...
            try {
                ...
                if (clientFormError) {
                    ...
                    clientFormError.classList.remove(
                        "hidden"
                    );
                }
    ...
    archiveClientCancel?.addEventListener(
        "click",
        closeArchiveClientModal
    );

    archiveClientModal?.addEventListener(
        "click",
        event => {
    ...
    archiveClientConfirm?.addEventListener(
        "click",
        async () => {
            if (!selectedClientId) {
                return;
    ...
    function setClientBrandsView(view) {
        clientBrandsLoading?.classList.toggle(
            "hidden",
            view !== "loading"
        );

        clientBrandsEmpty?.classList.toggle(
            "hidden",
            view !== "empty"
        );

        clientBrandsList?.classList.toggle(
            "hidden",
            view !== "list"
        );
    }
    ...
    function renderClientBrandOptions(clientId) {
        ...
        brands.forEach(brand => {
            const isAssignedToCurrentClient =
                brand.client_id === clientId;

            const assignedClientName =
                getBrandClientName(brand.client_id);
            ...
            if (isAssignedToCurrentClient) {
                relationshipText =
                    "Associada a este cliente";
            } else if (assignedClientName) {
                relationshipText =
                    `Associada a ${assignedClientName}`;
            }
    ...
    function openClientBrandsModal(clientId) {
        ...
        if (clientBrandsClientName) {
            clientBrandsClientName.textContent =
                client.name;
        }
    ...
    function setClientBrandsSubmitting(isSubmitting) {
        ...
        clientBrandsSave.disabled =
            isSubmitting;

        clientBrandsSave.setAttribute(
            "aria-busy",
            String(isSubmitting)
        );

        clientBrandsSave.textContent =
            isSubmitting
                ? "A guardar..."
                : "Guardar associações";
    }
    ...
    async function saveClientBrandAssociations() {
        ...
        const selectedBrandIds = Array.from(
            clientBrandsList.querySelectorAll(
                'input[data-brand-id]:checked'
            )
        ...
        const brandsToAssign =
            selectedBrandIds.filter(
                brandId =>
                    !currentBrandIds.includes(
                        brandId
                    )
            );

        const brandsToRemove =
            currentBrandIds.filter(
                brandId =>
        ...
        if (
            brandsToAssign.length === 0 &&
            brandsToRemove.length === 0
        ) {
            showToast(
                "info",
                "Nenhuma alteração",
                "As associações das marcas continuam iguais."
            );
        ...
        try {
            ...
            showToast(
                "success",
                "Marcas atualizadas",
                "As associações do cliente foram guardadas."
            );
        } catch (error) {
            console.error(
                "Erro ao atualizar marcas do cliente:",
                error
            );
            ...
            showToast(
                ...
                error?.message ||
                "Não foi possível atualizar as associações."
            );
        } finally {
    ...
    clientBrandsModalClose?.addEventListener(
        "click",
        closeClientBrandsModal
    );

    clientBrandsCancel?.addEventListener(
        "click",
        closeClientBrandsModal
    );

    clientBrandsModal?.addEventListener(
        "click",
        event => {
            if (
                event.target ===
    ...
    clientBrandsSave?.addEventListener(
        "click",
        saveClientBrandAssociations
    );
    ...
    function openArchiveClientModal(clientId) {
        if (!archiveClientModal) {
            console.error(
                'Elemento com id="archive-client-modal" não encontrado.'
            );
    ...
    function setArchiveClientSubmitting(isSubmitting) {
        ...
        archiveClientConfirm.disabled =
            isSubmitting;

        archiveClientConfirm.setAttribute(
            "aria-busy",
            String(isSubmitting)
        );

        archiveClientConfirm.textContent =
            isSubmitting
                ? "A arquivar..."
                : "Arquivar cliente";
    }
    ...
    async function archiveClient(clientId) {
        ...
        try {
            ...
            showToast(
                "success",
                "Cliente arquivado",
                "O cliente foi arquivado com sucesso."
            );
        } catch (error) {
            console.error(
    ...
    async function restoreClient(clientId) {
        try {
            ...
            showToast(
                "success",
                "Cliente restaurado",
                "O cliente voltou a ficar ativo."
            );
        } catch (error) {
            console.error(
    ...
    clientsSearchInput?.addEventListener(
        "input",
        applyClientFilters
    );

    clientsStatusFilter?.addEventListener(
        "change",
        applyClientFilters
    );

    clientsSortSelect?.addEventListener(
        "change",
        applyClientFilters
    );

    clearClientFiltersButton?.addEventListener(
        "click",
        () => {
    ...
    try {
        ...
        showToast(
            ...
            error?.message ||
            "Não foi possível iniciar a página de clientes."
        );
    }
