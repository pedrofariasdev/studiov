...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const toggleArchivedFoldersButton =
        $("toggle-archived-folders-button");
    ...
    function renderFolders() {
        ...
        toggleArchivedFoldersButton.innerHTML = `
            <i data-lucide="${
                showArchivedFolders ? "folder" : "archive"
            }"></i>
    ...
    function getFilteredAssets() {
        ...
        return assets.filter(asset => {
            ...
            const matchesBrand =
                brand === "all" || asset.brand_id === brand;
            ...
            const matchesType =
                type === "all" || asset.media_type === type;
        ...
        selectedFiles.forEach((file, index) => {
            ...
            row.innerHTML = `
                <div class="media-upload-file-icon">
                    <i data-lucide="${getFileIconFromMime(
                        file.type
                    )}"></i>
                </div>
    ...
    async function uploadSingleFile(file, index) {
        ...
        const preparedAsset = Array.isArray(preparedData)
            ? preparedData[0]
            : preparedData;
    ...
    function openAssetDetails(assetId) {
        ...
        mediaDetailsType.textContent =
            mediaTypeLabels[asset.media_type] ||
            asset.media_type;
    ...
    async function confirmArchive() {
        ...
        try {
            const isAsset =
                confirmationAction.type === "asset";

            const rpcName = isAsset
                ? "archive_media_asset"
    ...
    mediaFoldersGrid.addEventListener("click", event => {
        ...
        const folderCard = event.target.closest(
            "[data-folder-id]"
        );
